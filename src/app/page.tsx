import { UserButton, SignIn, SignOutButton, SignInButton } from "@clerk/nextjs";
import { BrainCircuit } from "lucide-react";

export default function Home() {
  return (
    <main className="min-h-screen flex flex-col items-center justify-center bg-black text-white p-6 text-center">
      <div className="fixed top-6 right-6">
        <SignIn><UserButton afterSignOutUrl="/" /></SignIn>
      </div>
      <BrainCircuit className="w-20 h-20 text-indigo-500 mx-auto mb-6" />
      <h1 className="text-6xl font-bold mb-4 text-white">Nexus Brain</h1>
      <p className="text-zinc-400 text-lg mb-8 text-white">Centro de Comando Estratégico.</p>
      <div className="pt-6">
        <SignIn>
          <a href="/painel" className="inline-block px-12 py-4 bg-indigo-600 text-white font-bold rounded-2xl shadow-xl shadow-indigo-500/40 active:scale-95 transition-all">
            ENTRAR NO PAINEL
          </a>
        </SignIn>
        <SignOutButton>
          <div className="pt-4">
            <SignInButton mode="modal">
              <button className="px-12 py-4 bg-white text-black font-bold rounded-2xl">Acessar</button>
            </SignInButton>
          </div>
        </SignOutButton>
      </div>
    </main>
  );
}
